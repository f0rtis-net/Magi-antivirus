...
/**
 ** \file parser.tab.h
 ** Define the  Melhior ::parser class.
 */
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // rule_declaration
      char dummy1[sizeof ( Melhior::Rule )];
    ...
    TOKEN_QUOTATIONS = 264,        // "quotations"
    TOKEN_OPEN_EXP_BRAK = 265,     // "open_brak"
    TOKEN_CLOSE_EXP_BRAK = 266,    // "close_brak"
    TOKEN_COLON = 267,             // "colon"
    TOKEN_RULE_LEXEME = 268,       // "rule"
    TOKEN_CONDITIONS = 269,        // "cond"
    TOKEN_AND = 270,               // "and"
    TOKEN_OR = 271,                // "or"
    TOKEN_NOT = 272                // "not"
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_COMMA = 8,                             // "comma"
        S_QUOTATIONS = 9,                        // "quotations"
        S_OPEN_EXP_BRAK = 10,                    // "open_brak"
        S_CLOSE_EXP_BRAK = 11,                   // "close_brak"
        S_COLON = 12,                            // "colon"
        S_RULE_LEXEME = 13,                      // "rule"
        S_CONDITIONS = 14,                       // "cond"
        S_AND = 15,                              // "and"
        S_OR = 16,                               // "or"
        S_NOT = 17,                              // "not"
        S_YYACCEPT = 18,                         // $accept
        S_program = 19,                          // program
        S_bool_expr = 20,                        // bool_expr
        S_rule_declaration = 21,                 // rule_declaration
        S_rule_body = 22,                        // rule_body
        S_rule_expr = 23,                        // rule_expr
        S_byte_seq = 24                          // byte_seq
      };
    };
        ...
        switch (this->kind ())
    {
      case symbol_kind::S_rule_declaration: // rule_declaration
        value.move<  Melhior::Rule  > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t,  Melhior::Rule && v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const  Melhior::Rule & v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      case symbol_kind::S_rule_declaration: // rule_declaration
        value.template destroy<  Melhior::Rule  > ();
        break;
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        YY_ASSERT (tok == token::TOKEN_END
                   ...
                   || (token::TOKEN_LEFTPAR <= tok && tok <= token::TOKEN_NOT));
#endif
      }
#if 201103L <= YY_CPLUSPLUS
    ...
    /// Build a parser object.
     Parser  (Melhior::Scanner &scanner_yyarg, Melhior::database_parser &driver_yyarg);
    virtual ~ Parser  ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_QUOTATIONS (location_type l)
      {
        return symbol_type (token::TOKEN_QUOTATIONS, std::move (l));
      }
#else
      static
      symbol_type
      make_QUOTATIONS (const location_type& l)
      {
        return symbol_type (token::TOKEN_QUOTATIONS, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_OPEN_EXP_BRAK (location_type l)
      {
        return symbol_type (token::TOKEN_OPEN_EXP_BRAK, std::move (l));
      }
#else
      static
      symbol_type
      make_OPEN_EXP_BRAK (const location_type& l)
      {
        return symbol_type (token::TOKEN_OPEN_EXP_BRAK, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_CLOSE_EXP_BRAK (location_type l)
      {
        return symbol_type (token::TOKEN_CLOSE_EXP_BRAK, std::move (l));
      }
#else
      static
      symbol_type
      make_CLOSE_EXP_BRAK (const location_type& l)
      {
        return symbol_type (token::TOKEN_CLOSE_EXP_BRAK, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_RULE_LEXEME (location_type l)
      {
        return symbol_type (token::TOKEN_RULE_LEXEME, std::move (l));
      }
#else
      static
      symbol_type
      make_RULE_LEXEME (const location_type& l)
      {
        return symbol_type (token::TOKEN_RULE_LEXEME, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_CONDITIONS (location_type l)
      {
        return symbol_type (token::TOKEN_CONDITIONS, std::move (l));
      }
#else
      static
      symbol_type
      make_CONDITIONS (const location_type& l)
      {
        return symbol_type (token::TOKEN_CONDITIONS, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
  ...
  private:
    ...
    // User arguments.
    Melhior::Scanner &scanner;
    Melhior::database_parser &driver;
    ...
    , location (that.location)
  {
    ...
    {
      case symbol_kind::S_rule_declaration: // rule_declaration
        value.copy<  Melhior::Rule  > (YY_MOVE (that.value));
        break;
   ...
   Parser ::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    {
      case symbol_kind::S_rule_declaration: // rule_declaration
        value.move<  Melhior::Rule  > (YY_MOVE (s.value));
        break;
